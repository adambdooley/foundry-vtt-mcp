  ...
  getToolDefinitions() {
    return [
      ...
      },
      {
        ...
        description:
          'The sensor tool: read the full tactical state of one actor or every combatant before deciding an action. Always returns HP, active conditions and effects (with remaining durations), initiative, and whose turn it is; returns AC, movement, spell slots, and consumable items where the game system exposes them. Provide exactly one of "actor" (any actor, combat not required) or "all: true" (requires an active combat; also returns round and turn).',
        inputSchema: {
          type: 'object',
          properties: {
            actor: {
